...
class TemporalBMLP(eqx.Module):
    ...
    def __call__(self):
        ...
        b0 = jax.nn.relu(out[0:1])  # Enforces positivity while keeping continuity
        b_half_raw = out[1: 2 * self.r_half + 1]
        b_half_raw = b_half_raw.reshape((self.r_half, 2))
        b_half_real = jax.nn.relu(b_half_raw[:, 0])
        b_half = b_half_real + 1j * b_half_raw[:, 1]
        return b0, b_half
...
class NeuralDMD(eqx.Module):
    ...
    def __init__(self, r, hidden_size=256, num_layers=4, key=None, num_frequencies=10,
                 temporal_latent_dim=16, temporal_hidden=64, temporal_layers=2):
        assert r % 2 == 0, "r must be even to ensure complex conjugate symmetry"
        self.r_half = r // 2
        self.scale = jnp.array(1e-3)
    ...
    def spatial_forward(self, xy):
        ...
        W0 = jax.nn.relu(jnp.expand_dims(output[0], axis=0))
...
# Loss Function
# -------------------------
def loss_fn(model, xy, frame_batch, target_vis_batch, num_vis_batch, A_batch, sigma_batch, time_indices, mask_batch, beta, frame_max, frame_min):
    """
    Args:
    ...
    intensities_raw = (2 * jnp.real(jnp.einsum('pr, rt, r -> pt', W_half, lambda_exp, b_half)) + W0[:, 0:1] * b0[0])
    intensities = intensities_raw * jax.nn.relu(model.scale)
    # intensities =  intensities_raw * (frame_max - frame_min) + frame_min
    ...
    # NOTE: uncomment to add orthogonality loss
    # total_loss += beta * orthogonality_loss
    return total_loss, (reconstruction_loss, beta * orthogonality_loss)
...
def train_step(model, opt_state, xy, frame_batch, target_vis_batch, num_vis_batch, A_batch, sigma_batch, time_indices, mask_batch, 
               optimizer, beta, frame_max, frame_min):
    (loss, aux), grads = eqx.filter_value_and_grad(loss_fn, has_aux=True)(
        model, xy, frame_batch, target_vis_batch, num_vis_batch, A_batch, sigma_batch, time_indices, mask_batch, beta, frame_max, frame_min
    )
...
def train_epoch_jit(model, opt_state, batch_list, optimizer, beta, key, frame_max, frame_min):
    ...
    def scan_fn(carry, batch_idx):
        ...
        new_model, new_opt_state, loss, rec_loss, ortho_loss, grads = train_step(
            ...
            time_indices, mask, optimizer, beta, frame_max, frame_min
        )
    ...
    with tqdm(total=num_epochs) as pbar:
        for epoch in range(num_epochs):
            ...
            model, opt_state, avg_loss, rec_loss, ortho_loss, grads = train_epoch_jit(
                model, opt_state, epoch_data, optimizer, beta, key, frame_max, frame_min
            )
            rec_losses.append(float(rec_loss))
            ortho_losses.append(float(ortho_loss))
            total_losses.append(float(avg_loss))
            # NOTE: uncomment to print gradients and parameter norms.
            # if epoch % 10 == 0:
